# Porting `journio-transact-golang` -> Rust

This document is the working plan for porting
[journio-transact-golang](../journio-transact-golang) to Rust with Python and Node
bindings. Keep the Go source open alongside; every Rust unit carries a
`// ported from journio/<file>.go:<line>` comment so diffs stay auditable.

## What the Go project is

Journio Transact: durable workflow orchestration on Postgres / CockroachDB /
SQLite. About 22K LOC (about 16K non-test). The interesting files, by size:

| Go file | LOC | Rust target |
|---|---|---|
| `journio/system_database.go` | 5,916 | `journio-core::system_db` trait + `journio-postgres` / `journio-sqlite` impls |
| `journio/workflow.go` | 5,269 | `journio-core::workflow` + `journio-core::context` |
| `journio/conductor.go` | 2,052 | `journio-conductor` |
| `journio/journio.go` | 958 | `journio-core::config` + `context::JournioContext` |
| `journio/client.go` | 879 | `journio-core::client` (done) |
| `journio/queue.go` | 803 | `journio-core::queue` |
| `journio/admin_server.go` | 618 | `journio-admin` (axum) (done) |
| `journio/debouncer.go` | 549 | `journio-core::debouncer` |
| `journio/dialect.go` | 418 | `journio-core::dialect` (done in scaffold) |
| `journio/scheduler.go` | 342 | `journio-core::scheduler` |
| `journio/serialization.go` | 373 | `journio-core::value` (done in scaffold) |
| `cmd/journio/*` | - | `journio-cli` (clap) — done (version/migrate/reset/workflow/start/init/postgres) |
| `journio/migrations/*.sql` (40) | - | embedded via `include_str!`, reused verbatim for PG |
...
1. **No reflection -> trait objects.** Go registers `Workflow[P,R]` by name via
   ...
   erased trait is the seam Python/Node bind to. -> `crates/journio-core/src/workflow.rs`
...
durable-wf-rust/
|- crates/
|  |- journio-core/        present: runtime, registry, context, config, errors, dialect
|  |- journio-postgres/    present: SystemDatabase impl, migrations, LISTEN/NOTIFY wakeups
|  |- journio-sqlite/      present: SystemDatabase impl, migrations, examples, integration tests
|  |- journio-admin/       present: axum HTTP server (workflow CRUD, recovery, queue metadata, global timeout, registered workflows, start, CORS)
|  |- journio-conductor/   planned: WS client (port `journio/conductor.go`)
|  `- journio-cli/         present: `journio` binary (version/migrate/reset/workflow/start/init/postgres)
|- examples/
...
Both bind to the **erased `Workflow` / `Step` traits** in `journio-core`. The Rust
runtime owns Tokio; the VM only supplies function bodies.

```text
user code (Python/Node)
   |  journio.workflow(name="x")(async def x(ctx, input): ...)
   v
binding registers a callback by name
   |  implements `dyn Workflow::run` -> marshal Value into VM, call, await, marshal back
   v
journio-core engine (registry, recovery, queues, scheduler) - pure Rust
   v
journio-postgres / journio-sqlite - pure Rust
```
...
1. `cargo add` deps into a new `crates/journio-postgres` crate; copy
   `journio/migrations/*.sql` into `migrations/` and embed with `include_str!`.
...
5. In `journio-core::context`, replace `todo!()` bodies for `run_as_step`,
...
6. Add an integration test with `testcontainers` Postgres that mirrors
   `journio/journio_test.go`'s simplest workflow.

## Current status

- `journio-core`: core runtime path is implemented, including recovery/replay,
  ...
  and the standalone `Client` API (`journio-core::client`).
- `journio-cli`: the `journio` binary (clap) implements `version`, `migrate`,
  ...
  from `--db-url`, `journio-config.yaml`, or `JOURNIO_SYSTEM_DATABASE_URL`.
- `journio-admin`: an axum HTTP server exposing the full Journio Console endpoint
...
- `ui`: a Nuxt 3 + Vue 3 + Tailwind dashboard ("Journio Console") that consumes
...
- `journio/client.go` -> `journio-core::client::Client`: enqueue, `list_workflows`
...
- `cmd/journio/*` -> `crates/journio-cli`: the `journio` binary (clap) with `version`,
  ...
  CLI). URL resolution: `--db-url` flag -> `database_url` in `journio-config.yaml`
  -> `JOURNIO_SYSTEM_DATABASE_URL` env. Auto-detects Postgres vs SQLite from the
  URL.
- `journio/admin_server.go` -> `crates/journio-admin`: an axum HTTP server over
  `JournioContext` exposing all Journio Console endpoints: health check, workflow
...
- `start_workflow` no longer requires local registration for enqueued/delayed
  ...
  executor to pick up).
- Added `JournioContext::recover_workflows`, `cancel_all_before`,
...
- Coverage: `journio-sqlite/tests/client.rs` (9 tests),
  `journio-postgres/tests/client.rs` (3 tests), `journio-cli/tests/cli.rs`
  (10 integration + 6 unit tests), and `journio-admin/tests/admin.rs` (12 tests).
...
- replay behavior for `journio.recv`, `journio.getEvent`, and `journio.sleep`;
...
`journio-sqlite/tests/client.rs` and `journio-postgres/tests/client.rs`. The CLI (`version`/`migrate`/`reset`/`workflow`/`start`/`init`/`postgres`) is covered by
`journio-cli/tests/cli.rs`. The admin HTTP server is covered by
`journio-admin/tests/admin.rs`.
