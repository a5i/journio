//! `dbos reset` — ported from `cmd/dbos/reset.go`.
//!
//! Resets the DBOS system database. For Postgres this drops and recreates the
...
pub async fn run(
    ...
    if !skip_confirmation {
        let prompt = "This command resets your DBOS system database, deleting \
...
async fn reset_postgres(database_url: &str) -> Result<(), String> {
    ...
    let sanitized = dbos_postgres::sanitize_identifier(&db_name);
    if is_crdb {
