//! `dbos init` — ported from `cmd/dbos/init.go`.
//!
//! Scaffolds a new Rust DBOS application from the embedded starter template
//! (the Go version scaffolds Go code; this port scaffolds Rust code that uses
//! `dbos-core` + `dbos-sqlite`).
//!
...
const DEFAULT_PROJECT_NAME: &str = "dbos-rust-starter";
...
pub fn run(project_dir: Option<&str>) -> Result<(), String> {
    ...
    println!("Created new DBOS application: {dir}");
    println!();
    ...
    println!();
    println!("Then try:");
    println!("  dbos --db-url sqlite://{name}.db workflow list");
    println!("  dbos --db-url sqlite://{name}.db workflow steps <workflow-id>");
    Ok(())
}
...
const TEMPLATE_FILES: &[(&str, &str)] = &[
    ...
    ("dbos-config.yaml", "dbos-config.yaml"),
    ("README.md", "README.md"),
];
...
fn include_str_template(path: &str) -> &'static str {
    match path {
        ...
        "dbos-config.yaml" => include_str!("../../templates/starter/dbos-config.yaml"),
