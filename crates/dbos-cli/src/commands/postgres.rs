//! `dbos postgres` — ported from `cmd/dbos/postgres.go`.
//!
...
/// `dbos postgres stop` — stop the local Postgres container.
...
/// Get the password from `PGPASSWORD` (default: "dbos").
fn password() -> String {
    std::env::var("PGPASSWORD").unwrap_or_else(|_| "dbos".to_string())
}
