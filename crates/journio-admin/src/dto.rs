...
//! because the Journio Console expects that exact shape. The `Input`/`Output`
...
use journio_core::{QueueConfig, StepRecord, WorkflowStatus};
...
fn parse_status(s: &str) -> Option<journio_core::WorkflowStatusType> {
    use journio_core::WorkflowStatusType::*;
...
pub fn request_to_filter(req: &ListWorkflowsRequest) -> journio_core::ListWorkflowsFilter {
    journio_core::ListWorkflowsFilter {
