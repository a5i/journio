...
//! SQLite-backed `JournioContext` with the admin server running on an ephemeral
...
async fn setup() -> (Arc<JournioContext>, String) {
    let url = temp_db_url();
...
async fn health_returns_healthy() {
    ...
    let resp: serde_json::Value = reqwest::get(format!("{base}/journio-healthz"))
        .await
        .expect("request")
        .json()
        .await
...
async fn fork_workflow_returns_new_id() {
    ...
    backend
        ...
        .expect("result");
    backend
        .record_step_output(&journio_core::StepRecord {
...
async fn global_timeout_cancels_workflows_before_cutoff() {
    ...
    let status = client
        .post(format!("{base}/journio-global-timeout"))
...
async fn index_endpoint_returns_service_info() {
    ...
    let resp: serde_json::Value = reqwest::get(&base)
        ...
        .await
        .expect("json");
    assert_eq!(resp["service"], "journio-admin");
