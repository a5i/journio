//! Workflow & step execution model — ported from `dbos/workflow.go`.
//!
...
pub trait Step: Send + Sync {
    fn name(&self) -> &str;
    async fn run(&self, ctx: &WorkflowContext) -> DbosResult<Interchange>;
}
...
/// Wraps a typed workflow function `F: Fn(WorkflowContext, P) -> BoxFuture<DbosResult<R>>`.
...
where
    ...
    R: Serialize + Send,
    F: Fn(WorkflowContext, P) -> BoxFuture<DbosResult<R>> + Send + Sync,
{
    ...
    F: Fn(WorkflowContext, P) -> BoxFuture<DbosResult<R>> + Send + Sync,
{
    ...
    async fn run(&self, ctx: &WorkflowContext, input: Interchange) -> DbosResult<Interchange> {
        let typed: P = serde_json::from_value(input).map_err(|e| DbosError {
            code: DbosErrorCode::WorkflowUnexpectedTypeError,
        ...
        serde_json::to_value(result).map_err(|e| DbosError {
            code: DbosErrorCode::WorkflowUnexpectedTypeError,
...
/// Wraps a typed step function `F: Fn(WorkflowContext) -> BoxFuture<DbosResult<R>>`.
...
where
    R: Serialize + Send,
    F: Fn(WorkflowContext) -> BoxFuture<DbosResult<R>> + Send + Sync,
{
    pub fn new(name: impl Into<String>, func: F) -> Self {
        Self {
            name: name.into(),
    ...
    F: Fn(WorkflowContext) -> BoxFuture<DbosResult<R>> + Send + Sync,
{
    ...
    async fn run(&self, ctx: &WorkflowContext) -> DbosResult<Interchange> {
        ...
        let result: R = fut.await?;
        serde_json::to_value(result).map_err(|e| DbosError {
            code: DbosErrorCode::StepExecutionError,
...
impl Registry {
    ...
    pub fn register(&self, wf: Arc<dyn Workflow>) -> DbosResult<()> {
        let mut map = self
            .workflows
            .write()
            .map_err(|e| DbosError::new(DbosErrorCode::InitializationError, e.to_string()))?;
...
mod tests {
    ...
    impl Workflow for NoopWorkflow {
        ...
        async fn run(&self, _ctx: &WorkflowContext, input: Interchange) -> DbosResult<Interchange> {
            Ok(input)
        }
    }
    ...
    fn registry_rejects_duplicate_registration() {
        ...
        assert_eq!(err.code, DbosErrorCode::ConflictingRegistrationError);
    }
