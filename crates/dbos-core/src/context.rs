//! Runtime contexts & handles — ported from `dbos/dbos.go` (`DBOSContext`,
//! `dbosContext`) and `dbos/workflow.go` (`workflowState`, `WorkflowHandle`).
//!
//! The Go `DBOSContext` is an interface with ~30 methods; here it splits into:
//!   - [`DbosContext`]     — the long-lived runtime (registry, db, loops)
...
impl WorkflowContext {
    ...
    pub async fn run_as_step(&self, step: Arc<dyn Step>) -> DbosResult<Interchange> {
            ...
            Ok(output) => {
                dbos.system_db
                    .record_step_output(&StepRecord {
                        ...
                        output: Some(encode_interchange(&dbos, &output)?),
                        error: None,
    ...
    pub async fn run_workflow(&self, name: &str, input: Interchange) -> DbosResult<WorkflowHandle> {
    ...
    /// as a checkpointed step (`DBOS.sleep`); on replay, decodes the recorded
    /// deadline and sleeps only the remaining duration.
    pub async fn sleep(&self, duration: Duration) -> DbosResult<Duration> {
        const STEP_NAME: &str = "DBOS.sleep";
        let step_id = self.next_step_id();
        let dbos = self.dbos()?;
        ...
        let encoded = encode_interchange(&dbos, &serde_json::Value::String(deadline.to_rfc3339()))?;
        dbos.system_db
            .record_step_output(&StepRecord {
                workflow_uuid: self.workflow_id().to_string(),
                function_id: step_id,
    ...
    /// checkpointed step (`DBOS.recv`) so replay returns the same value;
    /// timeouts are recorded as errors for the same reason.
    pub async fn recv(&self, topic: &str, timeout: Duration) -> DbosResult<Interchange> {
        const STEP_NAME: &str = "DBOS.recv";
        let step_id = self.next_step_id();
        let dbos = self.dbos()?;
        ...
        loop {
            if let Some(notification) = dbos
                .system_db
                .consume_notification(self.workflow_id(), topic)
                .await?
            {
                let encoded = encode_interchange(&dbos, &notification.message)?;
                dbos.system_db
                    .record_step_output(&StepRecord {
                        workflow_uuid: self.workflow_id().to_string(),
                        function_id: step_id,
    ...
    /// Runs as a checkpointed step (`DBOS.send`) so the side effect happens
    /// exactly once across replays.
    pub async fn send(&self, dest: &str, msg: Interchange, topic: &str) -> DbosResult<()> {
        const STEP_NAME: &str = "DBOS.send";
        let step_id = self.next_step_id();
        let dbos = self.dbos()?;
        ...
        dbos.system_db.send(dest, topic, &msg).await?;
        dbos.system_db
            .record_step_output(&StepRecord {
                workflow_uuid: self.workflow_id().to_string(),
                function_id: step_id,
    ...
    /// checkpointed step (`DBOS.setEvent`).
    pub async fn set_event(&self, key: &str, value: Interchange) -> DbosResult<()> {
        const STEP_NAME: &str = "DBOS.setEvent";
        let step_id = self.next_step_id();
        let dbos = self.dbos()?;
    ...
    /// found is recorded as a checkpointed step (`DBOS.getEvent`); timeouts are
    /// recorded as errors.
    pub async fn get_event(
        ...
        key: &str,
        timeout: Duration,
    ) -> DbosResult<Interchange> {
        const STEP_NAME: &str = "DBOS.getEvent";
        let step_id = self.next_step_id();
        let dbos = self.dbos()?;
        ...
        let deadline = Instant::now() + timeout;
        loop {
            if let Some(value) = dbos.system_db.get_event_value(target, key).await? {
                let encoded = encode_interchange(&dbos, &value)?;
                dbos.system_db
                    .record_step_output(&StepRecord {
                        workflow_uuid: self.workflow_id().to_string(),
                        function_id: step_id,
    ...
    pub async fn write_stream(&self, key: &str, value: Interchange) -> DbosResult<()> {
        const STEP_NAME: &str = "DBOS.writeStream";
        let step_id = self.next_step_id();
        let dbos = self.dbos()?;
        ...
        dbos.system_db
            .write_stream(
                ...
                &encoded,
                step_id,
                Some(dbos.config.serializer.name()),
            )
            .await?;
        dbos.system_db
            .record_step_output(&StepRecord {
    ...
    pub async fn close_stream(&self, key: &str) -> DbosResult<()> {
        const STEP_NAME: &str = "DBOS.closeStream";
        let step_id = self.next_step_id();
        let dbos = self.dbos()?;
    ...
    pub async fn debounce_workflow(
        ...
        let target_workflow_id = match options.workflow_id.take() {
            Some(value) => value,
            None => durable_uuid_step(self, "DBOS.debounce.assignWorkflowID").await?,
        };
        let message_id = durable_uuid_step(self, "DBOS.debounce.assignMessageID").await?;
        let dbos = self.dbos()?;
        dbos.debounce_workflow_inner(
            workflow_name,
            key,
            delay,
            input,
    ...
    pub async fn patch(&self, patch_name: &str) -> DbosResult<bool> {
        let dbos = self.dbos()?;
        validate_patching_enabled(&dbos)?;
    ...
    pub async fn deprecate_patch(&self, patch_name: &str) -> DbosResult<()> {
        let dbos = self.dbos()?;
        validate_patching_enabled(&dbos)?;
        validate_patch_name(patch_name)?;
        validate_not_within_step(self)?;
    ...
    fn dbos(&self) -> DbosResult<Arc<DbosContext>> {
        self.inner.dbos.upgrade().ok_or_else(|| {
            DbosError::new(
                DbosErrorCode::InitializationError,
                "DBOS runtime is no longer available",
            )
        })
    }

    async fn recorded_step(&self, step_id: i32) -> DbosResult<Option<StepRecord>> {
        let dbos = self.dbos()?;
        let steps = dbos.system_db.get_steps(self.workflow_id()).await?;
...
pub struct WorkflowHandle {
    ...
    pub(crate) dbos: Weak<DbosContext>,
}
...
impl WorkflowHandle {
    ...
    pub async fn get_result(&self, timeout: Option<Duration>) -> DbosResult<Interchange> {
        let deadline = timeout.and_then(|value| Instant::now().checked_add(value));

        loop {
            let status = match self.get_status().await {
                Ok(status) => status,
                Err(err) if err.code == DbosErrorCode::NonExistentWorkflowError => {
            ...
            match status.status {
                ...
                WorkflowStatusType::MaxRecoveryAttemptsExceeded => {
                    return Err(workflow_terminal_error(
                        DbosErrorCode::DeadLetterQueueError,
                        &self.workflow_id,
                        status.error.unwrap_or_else(|| {
                            format!(
    ...
    pub async fn get_status(&self) -> DbosResult<WorkflowStatus> {
        let dbos = self.dbos()?;
        dbos.system_db
    ...
    pub async fn cancel(&self) -> DbosResult<bool> {
        let dbos = self.dbos()?;
        dbos.cancel_workflow(&self.workflow_id).await
    }

    pub async fn resume(&self, queue_name: Option<&str>) -> DbosResult<bool> {
        let dbos = self.dbos()?;
        dbos.resume_workflow(&self.workflow_id, queue_name).await
    }

    pub async fn fork(&self, options: ForkWorkflowOptions) -> DbosResult<WorkflowHandle> {
        let dbos = self.dbos()?;
        dbos.fork_workflow(&self.workflow_id, options).await
    }

    fn dbos(&self) -> DbosResult<Arc<DbosContext>> {
        self.dbos.upgrade().ok_or_else(|| {
            DbosError::new(
                DbosErrorCode::InitializationError,
                "DBOS runtime is no longer available",
            )
...
/// The long-lived DBOS runtime — ported from `dbosContext` (`dbos/dbos.go:221`).
///
/// Construct with [`DbosContext::new`] (which calls `process_config`), then
/// [`DbosContext::launch`] to start background loops and run recovery.
...
impl DbosContext {
    ...
    /// `NewDBOSContext`.
    pub async fn new(mut config: Config) -> DbosResult<Arc<Self>> {
    ...
    pub async fn shutdown(&self, _timeout: Duration) -> DbosResult<()> {
        self.cancel.cancel();
    ...
    pub async fn cancel_workflow(&self, workflow_id: &str) -> DbosResult<bool> {
        Ok(self
            .system_db
    ...
    pub async fn cancel_workflows(&self, workflow_ids: &[String]) -> DbosResult<Vec<String>> {
    ...
    pub async fn get_workflow_children(&self, workflow_id: &str) -> DbosResult<Vec<WorkflowStatus>> {
    ...
    ) -> DbosResult<(Vec<Interchange>, bool)> {
    ...
    pub async fn run_queue_once(self: &Arc<Self>, queue_name: &str) -> DbosResult<Option<WorkflowHandle>> {
    ...
    pub async fn run_all_queues_once(self: &Arc<Self>) -> DbosResult<usize> {
    ...
    async fn recover(self: &Arc<Self>) -> DbosResult<()> {
        let pending = self
            .system_db
    ...
    pub async fn get_workflow_steps(&self, workflow_id: &str) -> DbosResult<Vec<StepRecord>> {
    ...
    async fn debounce_workflow_inner(
        ...
        loop {
            ...
            match result {
                ...
                Err(err) if err.code == DbosErrorCode::QueueDeduplicated => {
    ...
    async fn execute_workflow(
        ...
        match workflow.run(&ctx, input).await {
            ...
            Err(err) => {
                let terminal_status = if err.code == DbosErrorCode::WorkflowCancelled {
                    WorkflowStatusType::Cancelled
                } else {
                    WorkflowStatusType::Error
                };
    ...
    async fn run_scheduler_once(self: &Arc<Self>) -> DbosResult<()> {
...
fn validate_schedule_spec(spec: &str) -> DbosResult<()> {
    parse_schedule(spec).map(|_| ())
}

fn parse_schedule(spec: &str) -> DbosResult<Schedule> {
...
fn resolve_backend(config: &Config) -> DbosResult<Arc<dyn SystemDatabase>> {
    if let Some(db) = config.system_db.clone() {
        return Ok(db);
    }
    Err(DbosError::new(
        DbosErrorCode::InitializationError,
        "no system_db provided: the dbos-postgres / dbos-sqlite crate must construct the backend \
         and inject it via Config.system_db (core is driver-free)",
    ))
}

fn encode_interchange(dbos: &DbosContext, value: &Interchange) -> DbosResult<String> {
    dbos.config.serializer.serialize(value)
}
...
fn decode_stream_entry(
    ...
    dbos.config
        ...
        .map_err(|err| {
            DbosError::new(
                DbosErrorCode::WorkflowUnexpectedTypeError,
                format!(
                    "failed to decode stream entry for workflow {workflow_id}, key {key} with serialization {:?}: {err}",
                    entry.serialization.as_deref().unwrap_or(dbos.config.serializer.name())
                ),
            )
        })
}
...
fn duration_to_millis_u64(value: Duration) -> DbosResult<u64> {
...
fn validate_queue_options(queue_name: &str, options: &QueueOptions) -> DbosResult<()> {
    if queue_name.is_empty() {
        return Err(DbosError::new(
            DbosErrorCode::InitializationError,
            "queue_name cannot be empty",
...
fn parse_debouncer_input(workflow: &WorkflowStatus) -> DbosResult<DebouncerWorkflowInput> {
...
async fn durable_uuid_step(ctx: &WorkflowContext, name: &'static str) -> DbosResult<String> {
...
async fn durable_now_step(ctx: &WorkflowContext, name: &'static str) -> DbosResult<DateTime<Utc>> {
    let step = Arc::new(crate::workflow::StepFunc::new(name, |_ctx| {
...
impl Workflow for InternalDebouncerWorkflow {
    ...
    async fn run(&self, ctx: &WorkflowContext, input: Interchange) -> DbosResult<Interchange> {
        ...
        let start_time = durable_now_step(ctx, "DBOS.debounce.startTime").await?;
        ...
        loop {
            let now = durable_now_step(ctx, "DBOS.debounce.loopTime").await?;
            ...
            match ctx.recv(DEBOUNCER_TOPIC, remaining).await {
                ...
                Err(err) if err.code == DbosErrorCode::TimeoutError => break,
                Err(err) => return Err(err),
            }
        }
...
fn validate_patching_enabled(dbos: &DbosContext) -> DbosResult<()> {
    if dbos.config.enable_patching {
        return Ok(());
    }
    Err(DbosError::new(
        DbosErrorCode::PatchingNotEnabled,
        "Patching system is not enabled. Set enable_patching to true in the DBOS context configuration to use patch and deprecate_patch",
    ))
}

fn validate_patch_name(patch_name: &str) -> DbosResult<()> {
    if patch_name.is_empty() {
        return Err(DbosError::new(
            DbosErrorCode::StepExecutionError,
...
fn validate_not_within_step(ctx: &WorkflowContext) -> DbosResult<()> {
...
/// Build a `DbosError` anchored at a workflow step — mirrors Go's
...
fn ctx_step_error(
    ...
    let mut err = DbosError::new(code, message);
    err.workflow_id = Some(workflow_id.to_string());
    err.step_id = Some(step_id);
    err.step_name = Some(step_name.to_string());
    err
...
fn parse_deadline(value: &Interchange) -> DbosResult<DateTime<Utc>> {
    value
        .as_str()
...
fn workflow_terminal_error(code: DbosErrorCode, workflow_id: &str, message: String) -> DbosError {
    let mut err = DbosError::new(code, message);
    err.workflow_id = Some(workflow_id.to_string());
    err
}
...
mod tests {
    ...
    /// Matches `_DBOS_NULL_TOPIC` / `NULL_TOPIC` in the real backends.
    ...
    impl SystemDatabase for FakeSystemDatabase {
        ...
        async fn init_workflow(
            &self,
            init: InitWorkflow,
        ) -> DbosResult<crate::system_db::InitWorkflowResult> {
        ...
        async fn consume_notification(
            ...
            workflow_id: &str,
            topic: &str,
        ) -> DbosResult<Option<Notification>> {
            let mut state = self.state.lock().expect("fake db lock");
            ...
            if let Some(n) = candidate {
                ...
                Ok(Some(Notification {
                    ...
                    serialization: Some("DBOS_JSON".to_string()),
                }))
            } else {
                Ok(None)
            }
        ...
        async fn read_stream(
            ...
            key: &str,
            from_offset: i64,
        ) -> DbosResult<(Vec<crate::types::StreamEntry>, bool)> {
            let state = self.state.lock().expect("fake db lock");
        ...
        async fn delete_workflows_before(&self, _before: DateTime<Utc>) -> DbosResult<u64> {
            Ok(0)
        }
    ...
    async fn test_context() -> (Arc<DbosContext>, Arc<FakeSystemDatabase>) {
    ...
    async fn workflow_handle_times_out_for_pending_workflow() {
        ...
        assert_eq!(err.code, DbosErrorCode::TimeoutError);
    }
    ...
    async fn send_and_recv_within_workflow_round_trips_message() {
        ...
        assert_eq!(names, vec!["DBOS.send", "DBOS.recv"]);
        // The recv step carries the consumed message.
        let recv_step = steps
            .iter()
            .find(|s| s.function_name == "DBOS.recv")
            .expect("recv step");
        ...
        let workflow = Arc::new(WorkflowFn::new("waiter", |ctx, _input: ()| {
            Box::pin(async move {
                match ctx.recv("empty", Duration::from_millis(60)).await {
                    ...
                    Err(err) if err.code == DbosErrorCode::TimeoutError => {
    ...
    async fn get_event_times_out_and_records_error_for_replay() {
        ...
        let get_step = steps
            .iter()
            .find(|s| s.function_name == "DBOS.getEvent")
    ...
    async fn sleep_records_deadline_and_completes() {
        ...
        let sleep_step = steps
            .iter()
            .find(|s| s.function_name == "DBOS.sleep")
    ...
    async fn sleep_replay_does_not_block_when_deadline_has_passed() {
        ...
        fake.record_step_output(&StepRecord {
            workflow_uuid: workflow_id.clone(),
            function_id: 1,
            function_name: "DBOS.sleep".to_string(),
    ...
    async fn patch_records_marker_and_selects_new_code_path() {
        ...
        let steps = fake.get_steps(handle.workflow_id()).await.expect("steps");
        assert_eq!(steps.len(), 1);
        assert_eq!(steps[0].function_name, "DBOS.patch-my-patch");
    }
    ...
    async fn deprecate_patch_consumes_existing_marker_without_nondeterminism() {
        ...
        fake.record_step_output(&StepRecord {
            workflow_uuid: workflow_id.clone(),
            function_id: 1,
            function_name: "DBOS.patch-my-patch".to_string(),
            output: None,
            error: None,
            child_workflow_id: None,
        })
    ...
    async fn patch_returns_error_when_patching_is_disabled() {
        ...
        assert_eq!(err.code, DbosErrorCode::PatchingNotEnabled);
