//! Programmatic client — ported from `dbos/client.go` (879 LOC).
//!
//! [`Client`] is a thin facade over [`DbosContext`] for code that wants to
//! drive a DBOS system database from the outside: enqueue workflows, read
...
use crate::context::{
    parse_cron_schedule, DbosContext, EnqueueOptions, ForkWorkflowOptions, INTERNAL_QUEUE_NAME,
    ReadStreamOptions, WorkflowHandle,
};
use crate::error::{constructors, DbosError, DbosErrorCode, DbosResult};
use crate::types::{
...
/// A programmatic handle for a DBOS system database — ported from Go's
/// `Client` interface (`dbos/client.go`).
///
...
/// Go's `ClientScheduleInput` (`dbos/client.go`).
...
impl Client {
    ...
    pub async fn list_workflows(&self, filter: ListWorkflowsFilter) -> DbosResult<Vec<WorkflowStatus>> {
    ...
    // -- schedule management -----------------------------------------------
    //
    // Ported from the schedule block of Go's `Client` (`dbos/client.go:612`).
    ...
    pub async fn create_schedule(&self, input: ClientScheduleInput) -> DbosResult<()> {
    ...
    pub async fn apply_schedules(&self, schedules: Vec<ClientScheduleInput>) -> DbosResult<()> {
    ...
    pub async fn get_schedule(&self, schedule_name: &str) -> DbosResult<Option<WorkflowSchedule>> {
    ...
    ) -> DbosResult<Vec<WorkflowSchedule>> {
    ...
    pub async fn pause_schedule(&self, schedule_name: &str) -> DbosResult<()> {
    ...
    pub async fn resume_schedule(&self, schedule_name: &str) -> DbosResult<()> {
        self.require_schedule(schedule_name).await?;
        self.ctx
            .system_db
    ...
    pub async fn delete_schedule(&self, schedule_name: &str) -> DbosResult<()> {
    ...
    pub async fn list_application_versions(&self) -> DbosResult<Vec<VersionInfo>> {
    ...
    pub async fn get_latest_application_version(&self) -> DbosResult<Option<VersionInfo>> {
    ...
    pub async fn set_latest_application_version(&self, version_name: &str) -> DbosResult<()> {
    ...
    pub async fn shutdown(&self, timeout: Duration) -> DbosResult<()> {
        self.ctx.shutdown(timeout).await
    }

    // -- helpers -----------------------------------------------------------

    async fn require_schedule(&self, schedule_name: &str) -> DbosResult<WorkflowSchedule> {
    ...
    async fn latest_version_name(&self) -> DbosResult<Option<String>> {
        Ok(self
            .ctx
            .system_db
...
fn validate_schedule_input(input: &ClientScheduleInput) -> DbosResult<()> {
...
fn validate_schedule_input_indexed(input: &ClientScheduleInput, index: usize) -> DbosResult<()> {
    if input.schedule_name.is_empty() {
...
mod tests {
    ...
    fn schedule_from_input_normalizes_empty_queue_to_none() {
        let input = ClientScheduleInput {
            ...
            // crate requires seconds; DBOS's 5-field specs are a known
