//! Configuration — ported from `Config` in `dbos/dbos.go` and `ClientConfig`
//! in `dbos/client.go`.
//!
//! `SystemDBPool` / `SqliteSystemDB` from the Go struct become concrete
//! `SystemDatabase` implementations supplied by `dbos-postgres` /
//! `dbos-sqlite` (kept out of core to avoid backend deps here).
...
/// Initialization config — ported from `dbos.Config`.
#[derive(Clone)]
pub struct Config {
...
/// Ported from `processConfig` in `dbos/dbos.go`. Validates + applies defaults
/// + honors `DBOS__APPVERSION` / `DBOS__VMID` env overrides.
pub fn process_config(input: &mut Config) -> Result<(), DbosError> {
...
/// (used by `DbosContext` to pick a backend).
pub fn detect_configured_dialect(cfg: &Config) -> Result<DialectName, DbosError> {
    if let Some(db) = &cfg.system_db {
        return Ok(db.dialect().name());
    }
    let url = cfg.database_url.as_deref().ok_or_else(|| {
        DbosError::new(DbosErrorCode::InitializationError, "no database configured")
    })?;
    dialect::detect_dialect(url)
}
