//! System database abstraction — ported from `dbos/system_database.go` (5,916 LOC).
//!
//! Every persistence operation the engine needs is a method on this trait.
//! Concrete impls live in `dbos-postgres` (tokio-postgres + deadpool, with
//! LISTEN/NOTIFY) and `dbos-sqlite` (sqlx). Keeping the trait in core means the
