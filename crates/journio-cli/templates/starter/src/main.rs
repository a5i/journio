...
use journio_core::{Config, JournioContext, StepFunc, WorkflowFn};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let workflow = Arc::new(WorkflowFn::new("example_workflow", move |ctx, _input: ()| {
        Box::pin(async move {
            ...
            Ok::<_, journio_core::JournioError>("Workflow completed")
        })
    }));
    ...
    println!(
        "Watch it: journio --db-url sqlite://{{PROJECT_NAME}}.db workflow steps {}",
        handle.workflow_id()
    );
