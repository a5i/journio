//! `journio` CLI — ported from `cmd/journio/*` (cobra).
//!
...
use journio_cli::{backend, commands, config, output};

use std::sync::Arc;

use clap::{Parser, Subcommand};

/// Journio CLI — manage Journio workflows from the command line.
#[derive(Parser)]
#[command(name = "journio", version, about = "Journio CLI", long_about = None)]
struct Cli {
    /// Your Journio system database URL (overrides config/env).
    #[arg(short = 'D', long, global = true)]
    db_url: Option<String>,

    /// Config file path (default: ./journio-config.yaml).
    ...
    /// Database schema name (defaults to "journio"; Postgres only).
    #[arg(long, global = true)]
...
enum Command {
    ...
    /// Create Journio system tables (run migrations).
    Migrate {
        /// The role with which you will run your Journio application (Postgres
        /// schema grants).
        #[arg(short = 'r', long = "app-role")]
        app_role: Option<String>,
    },

    /// Reset the Journio system database.
    Reset {
    ...
    /// Manage Journio workflows.
    Workflow {
        #[command(subcommand)]
        sub: WorkflowCommand,
    },

    /// Start your Journio application using the start commands in 'journio-config.yaml'.
    Start,

    /// Initialize a new Journio application from a template.
    Init {
        /// Project name (defaults to 'journio-rust-starter').
...
impl BuildInfo {
    const fn from_env() -> Self {
        Self {
            version: match option_env!("JOURNIO_BUILD_VERSION") {
                Some(v) => v,
                None => "dev",
            },
            commit: match option_env!("JOURNIO_BUILD_COMMIT") {
                Some(v) => v,
                None => "",
            },
            built_at: match option_env!("JOURNIO_BUILD_DATE") {
                Some(v) => v,
                None => "",
            },
        }
...
async fn build_client(
    database_url: &str,
    schema: Option<&str>,
) -> Result<Arc<journio_core::Client>, String> {
