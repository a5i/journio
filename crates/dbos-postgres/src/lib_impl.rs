...
//! `dbos/system_database.go` (lifecycle + the first five target methods).
//!
...
/// `_DBOS_NULL_TOPIC` (`system_database.go:3333`).
...
const STREAMS_CHANNEL: &str = "dbos_streams_channel";
...
/// Go default is `DBOS_JSON` (`serialization.go:51`).
...
impl PostgresSystemDatabase {
    ...
    pub fn connect(database_url: &str, schema: &str) -> Result<Self, DbosError> {
...
fn init_err(e: impl std::fmt::Display) -> DbosError {
    DbosError::new(DbosErrorCode::InitializationError, e.to_string())
}

pub(crate) fn db_err(e: tokio_postgres::Error) -> DbosError {
    DbosError {
        code: if matches!(
            sqlstate(&e),
...
pub(crate) fn pool_err(e: deadpool_postgres::PoolError) -> DbosError {
    DbosError::new(DbosErrorCode::InitializationError, e.to_string())
}
