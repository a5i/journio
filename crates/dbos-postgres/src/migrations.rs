...
//! `dbos-transact-golang/dbos/migrations/`). Each is rendered against the
...
use dbos_core::DbosError;
use deadpool_postgres::Pool;

use crate::db_err;
use crate::dialect::sanitize_identifier;

/// Migration table name — ported from `_DBOS_MIGRATION_TABLE`.
const MIGRATION_TABLE: &str = "dbos_migrations";
...
// Embedded migration sources — 1:1 with `dbos/migrations/*.sql`.
const M01: &str = include_str!("../migrations/1_initial_dbos_schema.sql");
const M01_LN: &str = include_str!("../migrations/1_initial_dbos_schema_listen_notify.sql");
...
pub async fn run_migrations(pool: &Pool, schema: &str) -> Result<(), DbosError> {
...
async fn cleanup_invalid_indexes(pool: &Pool, schema: &str) -> Result<(), DbosError> {
    let san = sanitize_identifier(schema);
    let client = pool.get().await.map_err(crate::pool_err)?;
    let rows = client
        .query(
