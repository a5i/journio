//! Backend dialect abstraction — ported from `journio/dialect.go`.
//!
...
/// Mirrors `DialectName` in `journio/dialect.go`.
...
pub fn detect_dialect(database_url: &str) -> Result<DialectName, crate::JournioError> {
    let lower = database_url.to_ascii_lowercase();
    if lower.is_empty() {
        Err(crate::JournioError::new(
            crate::JournioErrorCode::InitializationError,
...
pub trait Dialect: Send + Sync {
    ...
    /// Qualified-table prefix, e.g. `"journio".` for Postgres or `""` for SQLite
